...
{
     $.getJSON(
            'https://taghash.co/atootbandhan/getapp_version_ios.php', 
			{
		    ...
		    function (json) {
				 ...
				 var vcode = json[0].versioncode;
                   console.log(vcode);
				 if( parseInt(app_versionCode) < parseInt(vcode) )
				 {
	...
	$.ajax({
	 ...
	 console.log(data);
		  ...
		  {       
				  ...
				  listItems+= '<option class="searchlist_div_css" value='+data.data[i].Store_Code+' onclick=javascript:SelectedDealerName(\''+data.data[i].Store_Code+'\');>' + data.data[i].Store_Name + ' ' + '('+data.data[i].Store_Code+')'+ '</option>';
